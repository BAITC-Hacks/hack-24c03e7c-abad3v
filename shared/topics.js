...
function sourcesForTopic(task) {
  const questions = new Map([...(task.questionHistory || []), ...(task.questions || [])].map((question) => [question.id, question.field]));
  const latest = new Map();
  for (const answer of task.answers || []) {
    const field = questions.get(answer.questionId) || (answer.questionId.startsWith('q:') ? answer.questionId.slice(2) : answer.questionId);
