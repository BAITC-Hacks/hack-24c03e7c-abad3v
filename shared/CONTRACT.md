...
Версия: 2.2. Контракт редактора с автосохранением, источниками предложений, свободными темами и отдельной опубликованной версией. Совместим с `lastAnalysis`, метаданными анализа и большими исходными текстами из backend 1.3. Полный план — `../HACKATHON_PLAN.md`. Backend доступен в `../server/`; frontend подключается к описанному API.
...
`topic`: произвольная непустая строка до 80 символов, например «Поиск свободных аудиторий» или «Учёт оборудования». Пробелы по краям удаляются, последовательности пробельных символов превращаются в один пробел. В POST поле необязательно; до определения темы используется `other`. Прежние значения `education`, `career`, `operations`, `analytics`, `other` остаются допустимыми и сохраняются в данных CSV. Интерфейс может показывать их локализованные подписи; `other` означает «Тема не определена».

Создание задачи начинается с описания без обязательного выбора темы. AI предлагает тему отдельно от Card в `suggestedTopic`; frontend применяет её через PATCH `topic` вместе с принятыми предложениями карточки. Фильтр каталога `topic` принимает любую такую строку (URL-encoded) и ищет точное совпадение с **опубликованной** темой. Черновая тема становится публичной только после confirm и publish.
...
`Question`: `{id,field,text,sourceRevision,options?:Array<{label:string,value:string}>}`. Варианты — предложения для явного выбора, а не установленные факты: они не входят в Card и не меняют рейтинг до ответа пользователя. `label` — короткая подпись (до 80 символов), `value` — полный редактируемый ответ (до 300 символов). Для доступности данных и типа срока используются канонические значения соответствующих полей; для контакта и точной даты массив пуст. Старые вопросы без options поддерживаются интерфейсом через локальные варианты. `Answer`: `{questionId,value,skipped}`; при выборе передаётся value варианта обычным PATCH, после редактирования — изменённый текст. При пропуске `value:null, skipped:true`; не превращать пропуск в выдуманный ответ.
...
`suggestedTopic` — предлагаемая тема для поиска, обычно 2–4 слова и не более 80 символов; список тем не ограничен. Значение null означает, что смысла недостаточно для определения темы либо тема защищена ручной правкой. Live-режим определяет тему по материалам задачи, шаблонный — локальными правилами из тех же материалов. Это предложение классификации, которое можно исправить; оно не является полем Card и не участвует в его рейтинге или массиве evidence. Новые AI-ответы всегда содержат suggestedTopic; в старых сохранённых результатах отсутствие поля сохраняется и не меняет текущую тему. Для нового результата null сбрасывает только незащищённую автоматически определённую тему в other. Версия AI-промпта обновлена, поэтому прежний кэш не используется для нового анализа. Анализ сохраняет предложение и кэш, но не изменяет topic или revision автоматически.

`manualFields` — список путей Card, изменённых пользователем, включая намеренно очищенные; дополнительно допустим маркер `topic` для ручной темы. Новый frontend **всегда** отправляет этот список, включая `[]`: PATCH заменяет список защиты и переносит объединение прежних ручных правок с новыми. Для совместимости со старым frontend при отсутствии `manualFields` изменённые поля и тема автоматически считаются ручными; неизменённые пустые поля не блокируются. Backend не перезаписывает защищённые поля при генерации и возвращает suggestedTopic:null при защите topic; frontend также объединяет предложение с рабочей карточкой и темой, сохраняя ручные и несохранённые конкурентные правки. Для снятия защиты передать новый список без нужного пути. AI не применяет proposal к workingCard или suggestedTopic к topic автоматически.
...
| `POST /api/tasks` | `{draftText,topic?}`; topic по умолчанию `other` | 201 `{task:OwnerTask}`; начать редактор |
| `GET /api/tasks?scope=mine` | — | `{items:TaskSummary[],total}`; рабочая область бизнеса |
| `GET /api/tasks?scope=catalog&topic=education&level=ready` | topic — свободная строка до 80 символов; фильтры и `limit/offset` необязательны | `{items:TaskSummary[],total}`; только опубликованные, сортировка `score DESC, publishedAt DESC` |
...
4. Каталог на `TaskSummary`: сортировку выполняет backend. Frontend загружает все страницы опубликованных задач, строит направления из их тем и локально применяет фильтры темы, готовности и поиск, сохраняя порядок сервера. Одинаковые подписи тем объединяются без учёта регистра и лишних пробелов. API также поддерживает отдельные фильтры `topic` и `level` для других клиентов.
