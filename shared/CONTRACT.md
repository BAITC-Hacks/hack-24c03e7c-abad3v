...
`Question`: `{id,field,text,sourceRevision,options?:Array<{label:string,value:string}>,origin?:"live"|"template",refines?:boolean,baseValue?:string}`. Варианты — предложения для явного выбора, а не установленные факты: они не входят в Card и не меняют рейтинг до ответа пользователя. `label` — короткая подпись (до 80 символов), `value` — полный редактируемый ответ (до 300 символов). Для доступности данных и типа срока используются канонические значения соответствующих полей; для контакта и точной даты массив пуст. Старые вопросы без options поддерживаются интерфейсом через локальные варианты. `Answer`: `{questionId,value,skipped}`; при выборе передаётся value варианта обычным PATCH, после редактирования — изменённый текст. При пропуске `value:null, skipped:true`; не превращать пропуск в выдуманный ответ.

`origin` отмечает происхождение конкретного вопроса: валидные факты модели сохраняются даже при неверном числе вопросов; недостающие вопросы дополняются локально и отмечаются `template`, с пояснением в warnings. Для уже заполненного текстового поля `refines:true` и `baseValue` задают исходные сведения. Ответ дополняет их: frontend сохраняет полную строку `baseValue + уточнение` в Answer, а в поле ввода показывает только добавляемую деталь. Пропуск/неизвестность уточнения восстанавливает основу, не отзывает исходный факт и не возвращает этот вопрос при следующем анализе. Ручные правки имеют приоритет. Если полный ответ длиннее поля карточки, он сохраняется целиком в Answer, а пользователь получает пояснение о ручном сокращении.

ID обычного вопроса стабилен для поля (`q:data.availability`; прежние ID сохраняются для совместимости). Уточнение заполненного поля получает ID `q:refine:<field>:<hash>`, зависящий от текста и основы, чтобы старый ответ не выглядел ответом на новое уточнение. `task.questions` содержит только текущую партию вопросов последнего analyze. Compose сохраняет эту партию без изменений. `questionHistory` хранит все известные вопросы, включая текущие и архивные; ответы связываются с полями через эту историю. Повторный анализ не теряет старые ответы. `Question.sourceRevision` — версия создания вопроса, верхнеуровневый `AiResult.sourceRevision` — версия анализируемой карточки. PATCH `answers` обновляет только переданные questionId; остальные ответы сохраняются. Обновлённые ответы перемещаются в конец массива: при нескольких архивных вопросах к одному полю используется последний ответ; для обычных вопросов пропуск или очистка отменяет прежнее значение, для refines отменяет только дополнение. Для очистки передать `{questionId,value:null,skipped:false}`. Ограничение 20 ответов снято; действуют предел 2000 символов на ответ и общий лимит JSON 64 КБ.
