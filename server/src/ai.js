...
function sourcesFor(task) {
  const sources = [{ id: 'draft', text: task.draftText }, ...latestAnswerSources(task).filter(({ field }) => field !== 'contact.channel')];
  for (const path of CARD_PATHS) {
    if (path === 'contact.channel') continue;
    const value = getField(task.workingCard, path);
    if (value) sources.push({ id: `card:${path}`, field: path, text: value });
  }
  return sources;
}

export function composeLiveResult(task, facts, sources = sourcesFor(task)) {
  ...
  for (const fact of facts) {
    ...
    if (getField(proposal, fact.field) !== null) evidence.push({ field: fact.field, sourceId: fact.sourceId, quote: fact.quote });
  }
...
export function prepareLiveRequest(task, mode) {
  ...
  const input = [
    ...
    { role: 'user', content: JSON.stringify({
      mode,
      sources: sourcesFor(task),
      protectedFields: task.protectedFields || [],
      filledFields: CARD_PATHS.filter((path) => getField(task.workingCard, path)),
...
export async function liveResult(task, mode, { prepared = prepareLiveRequest(task, mode), client } = {}) {
  ...
  return {
    result: { questions: parsed.questions.map(({ field, text }, index) => ({ id: `q${index + 1}`, field, text })), proposal, evidence, warnings: parsed.warnings },
