...
export function sourcesFor(task) {
  ...
  for (const field of CARD_PATHS) {
    ...
    if (value) sources.push({ id: `card:${field}`, field, text: value });
  }
  return sources;
}
...
export function startingProposal(task) {
  ...
  const withdrawnFields = new Set(latestAnswerStates(task).filter((answer) => answer.skipped || !answer.text?.trim()).map((answer) => answer.field));
...
export function addFact(task, result, field, value, sourceId, quote) {
  if (!value?.trim() || !quote?.trim() || protectedPaths(task).has(field)) return;
