...
function PublicTaskPage({ task, onBack, onToast }: { task: PublicTask; onBack: () => void; onToast: (message: string) => void }) {
  ...
  return <div className="page-content public-task-page">
    ...
    <div className="public-task-layout"><article className="public-task-article"><TaskSection title="Проблема" text={task.card.need} /><TaskSection title="Контекст" text={task.card.context} /><TaskSection title="Для кого" text={task.card.users} /><TaskSection title="Ожидаемый результат" text={[task.card.result.artifact, task.card.result.scope].filter(Boolean).join('\n')} /><TaskSection title="Критерии приёмки" text={[...new Set([task.card.success.metric, task.card.success.target].filter(Boolean))].join(' — ')} /><TaskSection title="Доступы и ограничения" text={task.card.constraints.technologyAccess} /><TaskSection title="Связь с бизнесом" text={[task.card.contact.channel, task.card.contact.consultation, task.card.contact.feedback].filter(Boolean).join('\n')} /></article>
      <aside className="public-task-aside"><section className="task-key-facts"><h2>Условия задачи</h2><dl><div><dt>Данные</dt><dd>{dataLabel(task.card.data.availability)}</dd>{task.card.data.source && <dd className="fact-extra">{task.card.data.source}</dd>}</div><div><dt>Срок</dt><dd>{deadlineLabel(task.card.constraints.deadlineMode === 'flexible' ? 'flexible' : task.card.constraints.deadlineDate)}</dd></div></dl></section>
...
function ResponseCard({ item, role, checked, onCompare, deciding, onDecide }: { item: NamedApplication; role: Role; checked: boolean; onCompare: () => void; deciding: string | null; onDecide: (status: 'selected' | 'rejected') => void }) {
  ...
  return <article className={`response-card ${checked ? 'is-selected' : ''}`}><header><div><h3>{role === 'business' ? item.teamName : item.taskTitle}</h3><span>Отправлено {formatDate(item.createdAt)}</span></div><ApplicationStatus status={item.status} /></header><div className="response-card-content"><div className="response-approach"><h4>Подход команды</h4><p>{item.idea}</p></div><div className="response-plan"><h4>План работы</h4><p>{item.plan}</p></div><div className="response-timeline"><h4>Сроки</h4><p>{item.timeline}</p>{link ? <a href={link} target="_blank" rel="noreferrer">Открыть прототип <Icon name="external" size={16} /></a> : <span className="response-no-prototype">Прототип не указан</span>}</div></div><footer>{role === 'business' && <label className="compare-checkbox"><input type="checkbox" checked={checked} onChange={onCompare} />Сравнить {item.teamName}</label>}{role === 'business' && <div className="response-decision">{item.status === 'pending' ? <><SecondaryButton disabled={!!deciding} onClick={() => onDecide('rejected')}>Отклонить</SecondaryButton><PrimaryButton disabled={!!deciding} onClick={() => onDecide('selected')}>{deciding === item.id ? 'Сохраняем…' : 'Выбрать команду'}</PrimaryButton></> : <span><Icon name="check" size={16} />Решение сохранено{formatDate(item.decidedAt) ? ` · ${formatDate(item.decidedAt)}` : ''}</span>}</div>}{role === 'team' && <span className="response-team-status">{item.status === 'pending' ? 'Бизнес рассматривает предложение.' : item.status === 'selected' ? 'Бизнес выбрал вашу команду.' : 'Бизнес отклонил предложение.'}</span>}</footer></article>
}
