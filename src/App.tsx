...
function App() {
  ...
  const createTask = async () => {
    setBusy(true)
    try {
      const response = await api.createTask('', 'education')
  ...
  return <div className="app-shell">
    <aside className="sidebar">
      ...
      <nav className="main-nav" aria-label="Главная навигация">
        {nav.map((item) => <button key={item.id} className={`nav-item ${page === item.id ? 'active' : ''}`} onClick={() => { setPage(item.id); setTaskId(null) }}><Icon name={item.icon} /><span>{item.label}</span>{item.id === 'applications' && role === 'business' && <span className="nav-count">5</span>}</button>)}
      ...
      <div className="sidebar-bottom">
        ...
        <div className="sidebar-profile"><div className="avatar avatar-dark">{actor.name.slice(0, 1)}</div><div className="profile-copy"><strong>{actor.name}</strong><span>{actor.profile}</span></div><span className="profile-menu">···</span></div>
      </div>
    </aside>
    ...
    <main className="main-area">
      <header className="topbar">
        ...
        <div className="topbar-right"><span className="demo-label"><span className="live-dot" />ДЕМО-РЕЖИМ</span><label className="actor-select-label" htmlFor="actor-switcher">Профиль</label><select id="actor-switcher" className="actor-select" value={actor.id} onChange={(event) => void changeActor(event.target.value)} disabled={busy}>
          <optgroup label="Бизнес">{demoActors.filter((item) => item.kind === 'business').map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</optgroup>
          <optgroup label="Студенческие команды">{demoActors.filter((item) => item.kind === 'team').map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</optgroup>
...
function TaskEditor({ taskId, onBack, onSaved, onApplications, showToast }: { taskId: string; onBack: () => void; onSaved: () => void; onApplications: () => void; showToast: (message: string) => void }) {
  ...
  const hydrate = (value: OwnerTask) => {
    setTask(value); setDraftText(value.draftText); setTopic(value.topic); setCard(structuredClone(value.workingCard)); setAnswers(value.answers || [])
  ...
  const runAi = async (mode: 'analyze' | 'compose') => {
    ...
    try {
      ...
      setAiResult(result); setAiMode(result.mode); setQuestions(result.questions)
...
function TaskDrawer({ task, onClose, onToast }: { task: PublicTask; onClose: () => void; onToast: (message: string) => void }) {
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      await api.createApplication(task.id, { idea, plan, timeline, prototypeUrl: prototypeUrl.trim() || null, clientRequestId: crypto.randomUUID() })
